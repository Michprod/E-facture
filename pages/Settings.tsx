...
const Settings: React.FC<SettingsProps> = ({ onLogout }) => {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-slate-900 dark:text-white text-3xl font-black tracking-tight mb-2">Company Settings</h1>
        <p className="text-slate-500 text-base max-w-2xl">Manage your company details, billing contacts, and security settings.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <section className="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden p-6">
              <div className="flex items-center gap-6 mb-8 pb-8 border-b border-slate-100 dark:border-slate-800">
                ...
                <div className="flex flex-col gap-1">
                  <p className="text-slate-900 dark:text-white text-lg font-bold">Company Logo</p>
                  <button className="mt-2 text-primary text-sm font-bold hover:underline self-start">Update Logo</button>
                </div>
              </div>
              <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex justify-end md:col-span-2 mt-4">
                  <button type="button" className="bg-primary text-white px-6 py-2 rounded-lg font-bold">Save Changes</button>
                </div>
              </form>
            </section>
          )}
          ...
          {activeTab === 'EUF' && (
            <section className="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800 mb-6">
                 <p className="text-sm text-blue-800 dark:text-blue-200 font-medium">Secure Connection</p>
                 <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">Credentials are stored with AES-256 encryption. For security reasons, the secret is masked.</p>
              </div>
              <div className="flex flex-col gap-6">
                ...
                <div className="flex justify-end gap-3 mt-4">
                   <button className="px-4 py-2 border border-slate-200 rounded-lg text-sm font-bold">Verify</button>
                   <button className="px-4 py-2 bg-primary text-white rounded-lg font-bold text-sm">Update</button>
                </div>
              </div>
            </section>
          )}
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
            <h3 className="text-sm font-bold mb-4">Account Actions</h3>
            <button 
              onClick={onLogout}
